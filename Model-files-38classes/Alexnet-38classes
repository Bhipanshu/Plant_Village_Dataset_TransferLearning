...
#running for particular number of epochs
...
for epoch in range(num_epochs):
    ...
    # Add the epoch's elapsed time to the total training time
    total_training_time += epoch_elapsed_time

total_training_time_minutes = total_training_time / 60
# Save the best model's state
best_model_filename = 'alexnet-38classes.pth'
torch.save(best_model_state, best_model_filename)

print(f"Total Training Time: {total_training_time_minutes:.2f} seconds")
print('Finished Training of AlexNet')


# calling model for testing

model = CustomAlexNet()

model_path = 'alexnet-38classes.pth'
...
print('Accuracy of the network on the test images: %d %%' % (
    100 * correct / total))
print(f'Total testing time : {total_testing_time_minutes:.2f}')
